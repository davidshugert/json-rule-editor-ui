...
import ValidateRulesTable from "../../components/validate/validate-rules-table";
...
const ValdiateContainer = ({ table, attributes, decisions, ruleset }) => {
  ...
  return (
    <div>
      {table && table.map((t,i) => {
        return <ValidateRulesTable key={i} attributes={attributes} decisions={decisions} ruleset={ruleset} index={i} />
      })}
...
class RulesetContainer extends Component {
  constructor(props) {
    super(props);
    this.state = { activeTab: "Validate", generateFlag: false };
  ...
  render() {
    const { attributes, decisions, name, table } = this.props.ruleset;
        ...
        <RuleErrorBoundary>
          ...
          <div className="tab-page-container">
            ...
            {this.state.activeTab === "Validate" && (
              <ValdiateContainer attributes={attributes} decisions={decisions} table={table} ruleset={this.props.ruleset}/>
            )}
